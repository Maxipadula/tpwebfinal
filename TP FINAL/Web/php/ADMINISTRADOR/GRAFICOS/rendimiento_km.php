	...
	<body>
		...
		<?php
			...
			if ($row = mysql_fetch_array($consulta_transporte)){
				...
				do{
					echo "<tr><td>".$row["marca"]."</td><td>".$row["modelo"]."</td><td>".$row["patente"]."</td><td>".$row["estado"]."</td><td>".$row["km"].".KM</td><td>".$row["chasis"]."</td><td>".$row["motor"]."</td><td class='tBotonAgregar'><a target='_blank' href='".$graficar_km ."?ID=".$row["ID"]."' class = 'tLink'>Seleccionar</a></td><br></tr> \n";     
